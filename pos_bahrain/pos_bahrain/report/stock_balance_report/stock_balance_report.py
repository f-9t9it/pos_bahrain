...
def execute(filters=None):
	...
	if filters.get('show_stock_ageing_data'):
		...
		item_wise_fifo_queue = FIFOSlots(filters, sle)
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Balance Value"), "fieldname": "bal_val", "fieldtype": "Currency", "width": 100, "convertible": "rate", "options": "currency","precision" : 5},
		...
		{"label": _("Valuation Rate"), "fieldname": "val_rate", "fieldtype": "Currency", "width": 90, "convertible": "rate", "options": "currency", "precision" : 5},
		# {"label": _("Valuation Rate"), "fieldname": "val_rate", "fieldtype": "Data", "width": 90},
	...
	for d in sle:
		...
		if key not in iwb_map:
			iwb_map[key] = frappe._dict({
				...
				"val_rate": "0.0", "bal_value" : 0.0
			})

		qty_dict = iwb_map[(d.company, d.item_code, d.warehouse)]
		qty_dict["val_rate"] = "0.0" 
		qty_dict["bal_val"] = "0.0" 
		...
		qty_dict.val_rate = "{:.5f}".format(Decimal(d.valuation_rate))
		qty_dict.bal_qty += qty_diff
		qty_dict.bal_value += value_diff
		qty_dict.bal_val = "{:.5f}".format(Decimal(qty_dict.bal_value))
...
def filter_items_with_no_transactions(iwb_map, float_precision):
	for (company, item, warehouse) in sorted(iwb_map):
		...
		for key, val in iteritems(qty_dict):
			if key != "val_rate" and key != "bal_val":
