...
def get_stock_ledger_entries(filters, items):
    item_conditions_sql = ''
    if items:
        item_conditions_sql = ' and sle.item_code in ({})'.format(', '.join([frappe.db.escape(i, percent=False) for i in items]))

    conditions = get_conditions(filters)
    date_filter = frappe.db.escape(filters.get('date'))
    ...
    sql_query = """
        SELECT
            ...
            IFNULL((SELECT sle.actual_qty 
                     FROM `tabStock Ledger Entry` sle 
                     WHERE sle.item_code = sle.item_code AND sle.posting_date = %s
    ...
    """ % (date_filter, item_conditions_sql, conditions)
