...
class CustomPreparedReport(Document):
	def before_insert(self):
		self.status = "Queued"
		self.report_start_time = frappe.utils.now()

	def enqueue_report(self):
		if self.ref_report_doctype == "Item Consumption Report":
			enqueue(run_background, prepared_report=self.name, timeout=6000, queue='short')
		else:
